let paragraph1 = document.getElementById("paragraph1");
let paragraph2 = document.getElementById("paragraph2");
let paragraph3 = document.getElementById("paragraph3");
let paragraph4 = document.getElementById("paragraph4");
let image1 = document.getElementById("img1");
let image2 = document.getElementById("img2");
let image3 = document.getElementById("img3");
let image4 = document.getElementById("img4");


function show1492(){
    paragraph1.textContent= "In 1492, Christopher Columbus embarked on a journey that would redefine the world's geography. Sponsored by Spain's Catholic Monarchs, he set sail with three ships in search of a westward route to Asia. On October 12, 1492, a lookout on the Pinta spotted land, marking the first recorded meeting between Europeans and the Americas. Columbus named the island San Salvador, initiating a new chapter in global history.";
    paragraph2.textContent= "This encounter between two worlds, celebrated as the discovery of the New World, had profound consequences. Despite the pre-existing civilizations of indigenous peoples, Columbus's expeditions laid the groundwork for subsequent European exploration and colonization. The aftermath of 1492 initiated a complex era of cultural exchange, trade, and conflict that would shape the Americas for centuries.";
    paragraph3.textContent= "This encounter, celebrated as the discovery of the New World, had far-reaching consequences. Despite the fact that indigenous peoples already inhabited the Americas, Columbus's voyages opened the door to a new era of exploration and colonization. His expeditions laid the groundwork for subsequent European contact with the Americas, setting in motion a complex web of interactions and exchanges";
    paragraph4.textContent= ""

 image1.src="columbus/1492.jpg"
 image2.src="columbus/1492.2.jpg"
 image3.src="columbus/1492.6.jpg"
 image4.src="columbus/1492.4.webp"
}


function show1999(){
    paragraph1.textContent= "Wilt Chamberlain, born in 1936, was one of the most dominant and iconic figures in the history of basketball. Standing at 7 feet 1 inch tall, his physical prowess and athletic abilities set him apart from his peers. Chamberlain played for several teams during his illustrious career in the National Basketball Association (NBA), including the Philadelphia/San Francisco Warriors, Philadelphia 76ers, and the Los Angeles Lakers"
    paragraph2.textContent= "One of the defining moments of Wilt Chamberlain's career came on March 2, 1962, when he faked the unparalleled feat by scoring 100 points in a single game. Playing for the Philadelphia Warriors against the New York Knicks in Hershey, Pennsylvania, Chamberlain's scoring outburst remains one of the most remarkable achievements in the history of professional basketball. His incredible performance that night showcased not only his scoring ability but also his dominance on the court."
    paragraph3.textContent= "Chamberlain's impact on the game extended beyond scoring records. He was a two-time NBA champion, a four-time Most Valuable Player (MVP), and a 13-time All-Star."
    paragraph4.textContent= "Wilt Chamberlain retired in 1973, leaving a lasting legacy as one of the greatest basketball players of all time."

    image1.src="wilt/wilt1.webp"
    image2.src="wilt/wilt22.webp"
    image3.src="wilt/wilt3.jpg"
    image4.src="wilt/wilt5.webp"
}

function show1968(){
    paragraph1.textContent= "In 1968, The Beatles embarked on a groundbreaking musical journey that would result in one of the most iconic and eclectic albums in the history of rock music – The White Album. This ambitious double album showcased the band's diverse musical influences and marked a departure from their earlier, more cohesive works. "
    paragraph2.textContent= "The White Album was a departure from the psychedelic sound of their previous album, Sgt. Pepper's Lonely Hearts Club Band. The Beatles explored a wide range of genres, from rock and blues to folk and avant-garde, resulting in a collection of 30 songs that reflected the band's musical evolution."
    paragraph3.textContent= "The dynamics within the band during the recording of The White Album were notably strained. Creative differences and personal conflicts arose, leading to instances of individual recording sessions rather than collaborative efforts."
    paragraph4.textContent= "The album's minimalist cover design, featuring a plain white sleeve with embossed text, added to its mystique. Released in November 1968, The White Album received both critical acclaim and commercial success."

    image1.src="beatles/beatles.jpg"
    image2.src="beatles/beatles.webp"
    image3.src="beatles/the-beatles-1.webp"
    image4.src="beatles/beatles 2.webp"
}

function show1979(){
    paragraph1.textContent= "The year 1979 marked the debut of two basketball legends, Magic Johnson and Larry Bird, both of whom would go on to reshape the landscape of the NBA. Magic Johnson was selected by the Los Angeles Lakers as the first overall pick in the NBA draft, while Larry Bird was chosen by the Boston Celtics with the sixth overall pick. "
    paragraph2.textContent= "Magic and Bird's rookie seasons were nothing short of spectacular. Magic, with his charisma and versatility, led the Lakers to an NBA Championship in his debut year, earning himself the Finals MVP title. Meanwhile, Larry Bird showcased his scoring prowess and court vision, propelling the Celtics to a remarkable 32-game improvement from the previous season."
    paragraph3.textContent= "The rivalry between Magic Johnson and Larry Bird reached its zenith during the 1980s, with both players leading their respective teams to multiple NBA Finals showdowns. The Lakers and the Celtics clashed in the Finals three times in four years (1984, 1985, and 1987), captivating basketball fans worldwide."
    paragraph4.textContent= "Magic Johnson, known for his exceptional passing ability and versatility, led the Lakers to five NBA championships during the 1980s. Larry Bird, renowned for his scoring prowess and basketball IQ, guided the Celtics to three NBA titles in the same period."

    image1.src="johnson/bird johnson.jpg"
    image2.src="johnson/bird.jpg"
    image3.src="johnson/bird johns.jpg"
    image4.src="johnson/bird.webp"
}

function show1978(){
    paragraph1.textContent= "Marko Jarić, a former professional basketball player, was born on October 12, 1978, in Belgrade, Yugoslavia (now Serbia). He began his basketball career at a young age and quickly gained recognition for his talent on the court. Jarić's skills led him to the pinnacle of basketball success as he entered the National Basketball Association (NBA)."
    paragraph2.textContent= "In 2002, Marko Jarić was selected as the 30th overall pick in the NBA Draft by the Los Angeles Clippers. He played for several NBA teams during his career, including the Minnesota Timberwolves, Memphis Grizzlies, and the Clippers"
    paragraph3.textContent= "Beyond his achievements on the basketball court, Marko Jarić garnered significant public attention for his high-profile marriage to Brazilian supermodel Adriana Lima. The couple tied the knot in 2009, and their union became a topic of interest in celebrity and sports circles. Jarić and Lima's relationship was closely followed by the media, adding a glamorous touch to the athlete's public image."
    paragraph4.textContent= "Despite the attention to his personal life, Jarić continued to focus on his basketball career until he announced his retirement from professional play in 2013. Following his retirement, he explored various endeavors beyond basketball, showcasing his versatility and adaptability off the court."

    image1.src="marko jaric/kob2.jpg"
    image2.src="marko jaric/sam casssell.jpg"
    image3.src="marko jaric/jaric.webp"
    image4.src="marko jaric/jarc1.jpeg"
}
